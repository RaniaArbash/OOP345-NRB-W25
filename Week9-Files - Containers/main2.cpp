...
public:
   ...
   bool operator==(const char* ename)const {// compare an employe to char array
...
int main() {
   ...
   for (auto& elm : E) {
    ...
    //char name[] = "Rania Simmons";
    
    it = find(E.begin(), E.end(), name);//
    
    if (it != E.end()){// very important
        cout << "Employee " << *it << " is found in this position " <<  it - E.begin() + 1 << endl;
    }else
        cout << "Thie name is not found" << endl;
    
    // take one employee at a time and comare it with name == false e > name
    
    
    // count how many employees has salary > 90k Note: Don't use normal loops
    int counter = count_if(E.begin(), E.end(), [](const Employee& e){
        return e < 30000.0;
    });
    cout << "The total number of employees with salary less than 30k is "<< counter << endl;
    
    
    
    //calculate the owing taxes by each one of them and save the taxes in another vector
